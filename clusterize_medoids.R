...
clusterize_kmedoids <- function(
  ...
  for(i in 1:max_at_least){
    ...
    for(j in 1:length(vec_aggregation)){
      ...
      cluster::pam(clrObj$CLR$x.clr,k = k,
                   metric = "euclidean", stand = FALSE,cluster.only = T)
      ...
      for(k in 1:max_k){
        dframe_clusters[,k] <- cluster::pam(
            clrObj$CLR$x.clr,k = k,
            metric = "euclidean", stand = FALSE,cluster.only = T)
      }
